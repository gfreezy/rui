...
use crate::text::movement::{movement, Movement};
use crate::text::selection::Selection;
use crate::text::storage::TextStorage;
use crate::text::text_input::{EditAction, MouseAction};
use druid_shell::kurbo::{Line, Point, Rect};
use druid_shell::piet::PietText;
use druid_shell::{Application, MouseEvent};
...
impl<T> Editor<T> {
    ...
    /// Create a new `Editor` with the provided text.
    // CHANGE: Addition over druid::Editor
    ...
    /// Returns whether the editor supports multi-line text.
    ///
    /// If this is false, inserted text will only insert up to the first
    /// included newline.
...
impl<T: TextStorage + EditableText + Clone> Editor<T> {
