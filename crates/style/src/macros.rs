    ...
    ($fun:tt, $ty:ty, [$($attr_name:tt => $parse_fun:tt => $field_ty:ty),* $(,)?]) => {
        ...
        pub(crate) fn $fun(input: &str) -> nom::IResult<&str, $ty> {
            ...
            let parser_tuple = alt!($(
                nom::sequence::tuple((
                    crate::utils::ws(parse_attr_name(stringify!($attr_name))),
                    crate::utils::ws(nom::bytes::complete::tag(":")),
                    crate::utils::ws(nom::combinator::map(
                        $parse_fun,
                        {
...
macro_rules! enum_type {
    ($enum_ty:tt => [$($variant:tt),*$(,)?], $default:tt, $parser_name:tt) => {
        ...
        pub(crate) fn $parser_name<'a>(input: &'a str) -> IResult<&'a str, $enum_ty> {
            alt!(
                $(
                    nom::combinator::map(crate::utils::parse_kebab_case(std::stringify!($variant)), |_| $enum_ty::$variant)
                ),*
            )(input)
        }
    };
