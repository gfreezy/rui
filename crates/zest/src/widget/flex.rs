...
use crate::{
    ...
    render_object::{
        render_box::{BoxConstraints, RenderBoxProps, RenderBoxWidget, Size},
...
impl Flex {
    ...
    pub fn build(self) -> RenderObject {
        let render = RenderFlex::create(self);
        RenderObject::new_render_box(Box::new(render))
    }
}
...
impl RenderBoxProps for RenderFlex {
    type Props = Flex;
    ...
    fn update(&mut self, ctx: &RenderObject, props: Flex) {
...
impl RenderBoxWidget for RenderFlex {
    fn compute_dry_layout(&mut self, ctx: &RenderObject, bc: BoxConstraints) -> Size {
        let sizes = self.compute_sizes(ctx, &bc, |child, bc| {
            child.get_dry_layout(bc.clone().into())
        });
        match self.direction {
